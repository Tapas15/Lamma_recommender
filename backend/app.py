...
async def get_candidate_recommendations(
    ...
    if availability:
        ...
        for candidate in candidates:
            ...
            # Check notice period or available_from
...
async def get_learning_recommendations(
    ...
    """
    Get learning recommendations for specified skills or career goals
    ...
    # If career goal is provided, get relevant skills for that role
    career_goal_skills = []
    if career_goal:
        # Get skills needed for the career goal
        career_goal_skills = get_skills_for_career_goal(career_goal)
        # Add these skills to our list
        skill_list.extend(career_goal_skills)
        # Remove duplicates while preserving order
        skill_list = list(dict.fromkeys(skill_list))
    
    # If no skills provided and no career goal, get from user profile
    if not skill_list and current_user["user_type"] == UserType.CANDIDATE:
        ...
        if candidate:
            ...
            # Try to get current role and industry from candidate experience
            experience = candidate.get("experience", [])
            if isinstance(experience, list) and len(experience) > 0:
                if isinstance(experience[0], dict):
                    target_role = experience[0].get("title", target_role)
                    industry = experience[0].get("industry", industry)
            
            skill_gap = await get_skill_gap_analysis(target_role, industry, "Mid-level", False, current_user)
    ...
    learning_resources = {
        ...
        "Leadership": [
            ...
            },
            {
                ...
                "description": "Guide for engineers pursuing technical leadership.",
        ...
        "Architecture": [
            ...
            },
            {
                ...
                "provider": "O'Reilly",
                "description": "An engineering approach to architecture fundamentals.",
    ...
    # Filter resources by timeframe
    timeframe_months = {
        "3_months": 3,
        "6_months": 6,
        "1_year": 12,
        "2_years": 24
    }.get(timeframe, 6)  # Default to 6 months
    ...
    for skill in skill_list:
        ...
        if skill_resources:
            # Filter resources by timeframe
            filtered_resources = filter_resources_by_timeframe(skill_resources, timeframe_months)
            resources.append({
                "skill": skill,
...
def get_skills_for_career_goal(career_goal):
    """Get skills needed for a specific career goal"""
    ...
    # Return skills for the career goal or empty list if not found
    return career_skills.get(career_goal, [])

def filter_resources_by_timeframe(resources, timeframe_months):
    """Filter learning resources based on timeframe"""
    filtered_resources = []
    
    for resource in resources:
        # Parse duration
        duration = resource.get("duration", "Ongoing")
        duration_months = 0
        ...
        if "week" in duration.lower():
            ...
            try:
                ...
                duration_months = weeks / 4  # Approximate weeks to months
            except:
                duration_months = 0.5  # Default if parsing fails
        elif "month" in duration.lower():
            # Extract months
            try:
                duration_months = int(duration.split()[0])
            except:
                duration_months = 1  # Default if parsing fails
        elif "ongoing" in duration.lower():
            duration_months = 1  # Assume 1 month for ongoing resources
        
        # Include resource if it fits within the timeframe or is ongoing
        if duration_months <= timeframe_months or "ongoing" in duration.lower():
            filtered_resources.append(resource)
    
    return filtered_resources

def create_learning_path(career_goal, timeframe_months, resources):
    """Create a structured learning path based on career goal and timeframe"""
    ...
    # Sort resources by priority (career goal skills first, then by duration)
    def resource_priority(item):
        ...
        # Career goal skills get higher priority (lower number)
        career_goal_priority = 0 if any(s["skill"] == skill and s.get("from_career_goal", False) for s in resources) else 1
        
        # Sort by career goal priority first, then by duration (shortest first)
        return (career_goal_priority, duration_months)
    
    # Sort resources by priority
    all_resources.sort(key=resource_priority)
    
    # Create learning path phases
    phases = []
    remaining_months = timeframe_months
    current_phase_months = 0
    current_phase = {
    ...
    for item in all_resources:
        ...
        # If adding this resource would exceed the phase limit, create a new phase
        if current_phase_months + duration_months > timeframe_months / 2 and current_phase["resources"]:
            phases.append(current_phase)
            remaining_months -= current_phase_months
            current_phase_months = 0
            
            # Create next phase name based on number of existing phases
            phase_name = "Phase 2: Intermediate"
            if len(phases) == 2:
        ...
        # Add resource to current phase if we have time left
        if current_phase_months + duration_months <= remaining_months:
            ...
            current_phase_months += duration_months
            current_phase["duration_months"] = current_phase_months
        
        # Stop if we've filled the timeframe
        if remaining_months <= 0:
            break
    
    # Add the last phase if it has resources
    if current_phase["resources"]:
        phases.append(current_phase)
...
def filter_resources_by_timeframe(resources, timeframe_months):
    ...
    for resource in resources:
        ...
        if "week" in str(duration).lower():
            # Convert weeks to months
            try:
                weeks = int(str(duration).split()[0])
                duration_months = weeks / 4  # Approximate weeks to months
            except:
                duration_months = 0.5  # Default if parsing fails
            ...
            try:
                ...
                duration_months = 1  # Default if parsing fails
        elif "ongoing" in str(duration).lower():
            duration_months = 1  # Assume 1 month for ongoing resources
        
        # Include resource if it fits within the timeframe or is ongoing
        if duration_months <= timeframe_months or "ongoing" in str(duration).lower():
            filtered_resources.append(resource)
...
@app.get("/ml/market-trends", response_model=dict)
async def get_market_trends(
    timeframe: str = "6_months",
    skill_category: str = "software_development",
    include_details: bool = True,
    current_user: dict = Depends(get_current_user)
):
    """Get predicted market trends for skills based on ML analysis"""
    try:
        # Validate timeframe
        valid_timeframes = ["3_months", "6_months", "1_year", "2_years", "5_years"]
        ...
        valid_categories = [
            "software_development", "data_science", "cloud_computing", 
            "cybersecurity", "devops", "mobile_development", "web_development",
            "artificial_intelligence", "blockchain", "iot", "ar_vr", "ui_ux",
            "project_management", "business_analysis", "product_management"
        ]
        
        if skill_category not in valid_categories:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Invalid skill category. Must be one of: {', '.join(valid_categories)}"
            )
        
        # Generate market trend predictions
        # In a production system, this would use actual ML models and market data
        trends = generate_market_trends(timeframe, skill_category, include_details)
        ...
        # Print the full exception for debugging
        import traceback
        print(f"Error in market trends analysis: {str(e)}")
        print(traceback.format_exc())
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error generating market trends: {str(e)}"
        )

def generate_market_trends(timeframe, skill_category, include_details):
    """Generate market trend predictions for the specified skill category and timeframe"""
    # Define category-specific skills and trends
    category_skills = {
        "software_development": [
            "Python", "JavaScript", "Java", "C#", "Go", "Rust", "TypeScript", 
            "Kotlin", "Swift", "PHP", "Ruby", "C++", "Scala"
        ],
        "data_science": [
            "Python", "R", "SQL", "TensorFlow", "PyTorch", "Pandas", "NumPy", 
            "Scikit-learn", "Tableau", "Power BI", "Big Data", "Hadoop", "Spark"
        ],
        "cloud_computing": [
            "AWS", "Azure", "Google Cloud", "Kubernetes", "Docker", "Terraform", 
            "CloudFormation", "Lambda", "S3", "EC2", "Azure Functions", "GCP"
        ],
        "cybersecurity": [
            "Network Security", "Penetration Testing", "SIEM", "Encryption", 
            "Security Auditing", "Vulnerability Assessment", "Threat Intelligence",
            "Identity Management", "Zero Trust", "Security Architecture"
        ],
        "devops": [
            "CI/CD", "Jenkins", "GitHub Actions", "Docker", "Kubernetes", "Ansible", 
            "Terraform", "Prometheus", "Grafana", "ELK Stack", "Infrastructure as Code"
        ],
        "web_development": [
            "React", "Angular", "Vue.js", "Next.js", "Node.js", "Express", "Django", 
            "Flask", "HTML5", "CSS3", "JavaScript", "TypeScript", "GraphQL", "REST API"
        ],
        "artificial_intelligence": [
            "Machine Learning", "Deep Learning", "NLP", "Computer Vision", 
            "Reinforcement Learning", "LLMs", "Neural Networks", "AI Ethics", 
            "Prompt Engineering", "RAG", "Vector Databases"
        ],
        "mobile_development": [
            "Swift", "Kotlin", "React Native", "Flutter", "iOS", "Android", 
            "Mobile UI Design", "Mobile Security", "App Performance Optimization"
        ],
        "ui_ux": [
            "UI Design", "UX Research", "Figma", "Adobe XD", "Sketch", "Wireframing", 
            "Prototyping", "User Testing", "Accessibility", "Design Systems"
        ],
        "project_management": [
            "Agile", "Scrum", "Kanban", "JIRA", "Confluence", "Risk Management", 
            "Stakeholder Management", "Project Planning", "Resource Allocation"
        ]
    }
    
    # Default to software development if category not found
    selected_skills = category_skills.get(skill_category, category_skills["software_development"])
    ...
    # Base growth rates that vary by timeframe
    ...
    # Generate trend data for each skill
    for skill in selected_skills:
        # Create a somewhat realistic but simulated growth trajectory
        # In a real system, this would use actual market data and ML predictions
        base_demand = random.uniform(0.5, 0.9)
        volatility = random.uniform(0.05, 0.2)
        growth_trend = random.uniform(-0.1, 0.3)  # Some skills might decline
        
        # Adjust growth based on skill category and current market trends
        category_factor = {
            "artificial_intelligence": 0.2,
            "cloud_computing": 0.15,
            "cybersecurity": 0.18,
            "data_science": 0.17,
            "devops": 0.12,
            "software_development": 0.1,
            "web_development": 0.08,
            "mobile_development": 0.07,
            "ui_ux": 0.05,
            "project_management": 0.03
        }.get(skill_category, 0.1)
        
        # Calculate projected growth rate
        growth_rate = (growth_trend + category_factor) * base_growth_multiplier
        
        # Calculate demand score
        current_demand = min(base_demand + (growth_rate * 0.2), 1.0)
        projected_demand = min(current_demand + (growth_rate * 0.5), 1.0)
        
        # Create trend object
        trend = {
            "skill": skill,
            "current_demand": round(current_demand * 100),
            "projected_demand": round(projected_demand * 100),
        ...
        if include_details:
            ...
            salary_growth = growth_rate * random.uniform(0.8, 1.2)
            
            # Generate industry relevance
            industries = ["Technology", "Finance", "Healthcare", "Retail", "Manufacturing", "Media"]
            industry_relevance = {}
            for industry in industries:
                industry_relevance[industry] = round(random.uniform(0.3, 0.9), 2)
            
            # Generate job role relevance
            job_roles = ["Junior Developer", "Senior Developer", "Architect", "Team Lead", "Manager"]
            role_relevance = {}
            for role in job_roles:
                role_relevance[role] = round(random.uniform(0.3, 0.9), 2)
            
            # Add details to trend object
            trend["details"] = {
                "salary_data": {
                    "current_avg": f"${avg_salary:,}",
                    "projected_avg": f"${int(avg_salary * (1 + salary_growth)):,}",
                    "growth_percentage": round(salary_growth * 100, 1)
                },
                "industry_relevance": industry_relevance,
                "role_relevance": role_relevance,
                "learning_curve": random.choice(["Steep", "Moderate", "Gradual"]),
                "complementary_skills": random.sample([s for s in selected_skills if s != skill], k=min(3, len(selected_skills)-1)),
                "market_factors": [
                    random.choice([
    ...
    # Sort trends by projected demand (descending)
    trends.sort(key=lambda x: x["projected_demand"], reverse=True)
    
    return trends

@app.get("/ml/learning-recommendations-public", response_model=dict)
async def get_ml_learning_recommendations_public(
    skills: str = None,
    career_goal: str = None,
    timeframe: str = "6_months"
):
    """
    Public version of the learning recommendations endpoint for testing.
    This endpoint provides personalized learning recommendations without requiring authentication.
    ...
    learning_resources = {
        ...
        "Architecture": [
            ...
            },
            {
                ...
                "provider": "SEI Series",
                "description": "Practical approach to software architecture.",
                "url": "https://www.amazon.com/Software-Architecture-Practice-3rd-Engineering/dp/0321815734",
                "duration": "3 months",
                "level": "Advanced"
            }
        ],
...
async def get_ml_learning_recommendations(
    ...
    # If no skills provided and no career goal, use candidate's skills if authenticated
    if not parsed_skills and not career_goal and current_user:
        try:
            # Get candidate's profile
            db = Database()
            candidate = await db.find_one(
                CANDIDATES_COLLECTION,
            ...
            # If there's an error, just continue with empty skills
            print(f"Error retrieving candidate skills: {str(e)}")
            pass
